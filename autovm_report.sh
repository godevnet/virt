...
## with logging
## vm00 vm01 vm02 vm03 vm04 vm05 vm06 vm07 vm08 vm9 vm0a vm0b vm0c vm0d vm0e vm0f vm10 vm11 vm12 vm13 vm14 vm15 vm16 vm17 vm18 v19 vm1a vm1b vm1c vm1d vm1e vm1f
	echo "#### Create and start $nb domain at one time ####"
	echo "$(df -h; free -m -h)" > /root/$uidtemp.log
install_info
elapsed
	echo "$(elapsed) : start" >> /root/$uidtemp.log
dom_erase
	elapsed
	echo "$(elapsed) : erase existing domains" >> /root/$uidtemp.log
temp_create
	elapsed
	echo "$(elapsed) : template created" >> /root/$uidtemp.log
# The 'clone' function call those tree sub-functions
clone
        elapsed
	echo "$(elapsed) : $nb domains created" >> /root/$uidtemp.log
temp_all_erase
	elapsed
	echo "$(elapsed) : $nb template erased" >> /root/$uidtemp.log
dom_start
	elapsed
	echo "$(elapsed) : $nb domains started" >> /root/$uidtemp.log
#dom_info
	echo "$(df -h; free -m -h)" >> /root/$uidtemp.log
	echo "$(elapsed) : total time" >> /root/$uidtemp.log
